...
/***/ function(module, exports, __webpack_require__) {
	...
	//  Title 
	d3.select(".heatmap").append("h2").text("Monthly Global Land Surface Temperature").attr("class", "title");

	// svg -  Main chart
	var svg = d3.select(".heatmap").append("svg").attr("height", fullHeight).attr("width", fullWidth);

	/* # Axis titles */

	// y-axis title
	svg.append("g").attr("transform", 'translate(25, ' + height / 2 + ') rotate(-90)').append("text").text("Months");

	// x-axis title
	svg.append("g").attr("transform", 'translate(' + width / 2 + ' , ' + (height + 60) + ')').append("text").text("Year");

	/*# Temperature variance Indentifier */

	var variance = [0, 2.7, 3.9, 5, 6.1, 7.2, 8.3, 9.4, 10.5, 11.6, 12.7];
	var colors = ["#5e4fa2", "#3288bd", "#66c2a5", "#abdda4", "#e6f598", "#ffffbf", "#fee08b", "#fdae61", "#f46d43", "#d53e4f", "#9e0142"];

	//  variance scale
	var vScale = d3.scaleLinear().domain(d3.extent(variance, function (d) {
		return d;
	})).range([0, variance.length * 50]);

	// color scale
	var cScale = d3.scaleOrdinal().domain(variance).range(colors);

	// identifier container
	var identifier = svg.append("g").attr("transform", 'translate(' + (fullWidth - variance.length * 50) + ', ' + (height + (margins.bottom - 50)) + ')');

	// rectangle colored identifier			
	identifier.selectAll("rect").data(variance).enter().append("rect").attr("height", 30).attr("width", 50).attr("fill", function (d) {
		return cScale(d);
	}).attr("x", function (d, i) {
		return vScale(i);
	});
	...
	// text identifier
	identifier.selectAll("text").data(variance).enter().append('text').text(function (d) {
		return d;
	}).attr("x", function (d, i) {
		return vScale(i) + 8;
	}) // plus 8 to center text
	.attr("fill", "#fff").attr("y", 50);

	/*# Tooltip*/

	var div = d3.select(".heatmap").append("div").attr("class", "tooltip").style("opacity", 0);

	var tooltip = function tooltip(d) {
		div.style('opacity', 0.8).html('\n\t\t\t\t\t\t\t<b>' + d.year + ' - ' + d.month + '</b>\n\t\t\t\t\t\t\t<br>\n\t\t\t\t\t\t\t' + d.variance + ' &#8451;\n\t\t\t\t\t\t').style("left", d3.event.pageX + 'px').style("top", d3.event.pageY + 'px');
	};

	/* # Data Renderer to svg*/
	var render = function render(rawData) {
		var data = rawData.monthlyVariance;
		var months = ["Janaury", "Febraury", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
		// changing monthly index number to month's name
		data.forEach(function (d, i) {
			months.forEach(function (m, j) {
				if (d.month - 1 === j) d.month = months[j];
			});
		});
		// y Scale - ( Months )
		var yScale = d3.scaleBand().domain(months).range([0, height]);

		// rendering y axis
		svg.append("g").attr("class", "y-axis").attr("transform", 'translate(' + margins.left + ', ' + margins.top + ')').call(d3.axisLeft(yScale));
		...
		svg.append("g").attr("class", "x-axis").attr("transform", 'translate(' + margins.left + ', ' + (height + 15) + ')').call(d3.axisBottom(xScale));
		// colors scale
		...
		svg.append("g").attr("transform", 'translate(' + margins.left + ', ' + margins.top + ')').selectAll("rect").data(data).enter().append("rect").attr("x", function (d) {
			return xScale(d.year);
		}).attr("y", function (d) {
			return yScale(d.month);
		}).attr("height", yScale.bandwidth()).attr("width", 15).style("cursor", "pointer").attr("fill", function (d) {
			return colorScale(d.variance);
		}).on("mouseover", tooltip).on("mouseout", function (d) {
			div.style("opacity", 0);
		});
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	// module
	exports.push([module.id, "* {\n  box-sizing: border-box;\n  font-family: sans-serif; }\n\nbody {\n  background: #925656; }\n\n.heatmap {\n  background: #3b543b;\n  margin: 10px auto;\n  height: 650px;\n  width: 1000px;\n  padding: 1px 0; }\n  .heatmap .title {\n    margin: 18px auto;\n    text-align: center;\n    color: white; }\n  .heatmap svg {\n    display: block;\n    margin: auto; }\n    .heatmap svg .y-axis path {\n      stroke: none; }\n    .heatmap svg .y-axis g line {\n      stroke: none; }\n    .heatmap svg .y-axis g text {\n      fill: white;\n      font-size: 18px; }\n    .heatmap svg .x-axis g text {\n      fill: white;\n      font-size: 15px; }\n  .heatmap .tooltip {\n    position: absolute;\n    height: 60px;\n    max-width: 150px;\n    text-align: center;\n    padding: 5px;\n    background: black;\n    border-radius: 5px;\n    color: white; }\n", ""]);
